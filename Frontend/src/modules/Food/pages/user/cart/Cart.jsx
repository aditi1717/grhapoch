...
const buildEffectiveCartPricing = ({
  ...
  if (hasServerPricing) {
    ...
    return {
      ...
      savings: Number.isFinite(Number(pricing.savings))
        ...
        : Math.max(0, subtotal + (Number.isFinite(serverDeliveryFee) ? serverDeliveryFee : 0) + (Number.isFinite(serverPlatformFee) ? serverPlatformFee : 0) + (Number.isFinite(serverTax) ? serverTax : 0) - serverTotal),
  ...
  const total = Math.max(0, subtotal + deliveryFee + platformFee + gstCharges - discount)
...
export default function Cart() {
  ...
  useEffect(() => {
    ...
    calculatePricing()
  }, [cart, pricingAddress, appliedCoupon, couponCode, restaurantId, restaurantData, scheduledOrderAt, replaceCart])
  ...
  }, [cart, pricing, feeSettings, pricingAddress, restaurantData, appliedCoupon])
                          ...
                          <>
                            ...
                            {otherSavings > 0 && (
                              <span className="text-[11px] bg-blue-100 dark:bg-blue-900/40 text-blue-700 dark:text-blue-300 px-2 py-0.5 rounded text-center ml-1 font-semibold border border-blue-200 dark:border-blue-800">
                                You saved {RUPEE_SYMBOL}{otherSavings.toFixed(0)}
                              </span>
                            )}
                          </>
                        ) : (
                  ...
                  <div className="mt-4 pt-4 border-t border-dashed border-gray-200 dark:border-gray-800 space-y-3">
                    ...
                    {itemDiscountAmount > 0 && (
                      <div className="flex justify-between text-sm text-[#EB590E] font-medium">
                        ...
                        <span>-{RUPEE_SYMBOL}{itemDiscountAmount.toFixed(2)}</span>
                      </div>
                    )}
                    <div className="flex justify-between text-sm">
