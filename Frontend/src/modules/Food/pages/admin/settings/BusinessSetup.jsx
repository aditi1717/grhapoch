...
export default function BusinessSetup() {
  ...
  const [formData, setFormData] = useState({
    ...
    pincode: "",
    region: "",
    adBannerDailyPrice: 100,
    adBannerMonthlyPrice: 2500,
    deliveryBoyRadius: 10,
    userVisibilityRadius: 10,
  });
    ...
    try {
      ...
      if (settings) {
        setFormData({
          ...
          adBannerDailyPrice: settings.adBannerDailyPrice ?? 100,
          adBannerMonthlyPrice: settings.adBannerMonthlyPrice ?? 2500,
          deliveryBoyRadius: settings.deliveryBoyRadius ?? 10,
          userVisibilityRadius: settings.userVisibilityRadius ?? 10,
        });
  ...
  const handleSave = async () => {
    try {
      ...
      const dataToSend = {
        ...
        adBannerDailyPrice: Number(formData.adBannerDailyPrice || 0),
        adBannerMonthlyPrice: Number(formData.adBannerMonthlyPrice || 0),
        deliveryBoyRadius: Number(formData.deliveryBoyRadius || 10),
        userVisibilityRadius: Number(formData.userVisibilityRadius || 10),
      };
        ...
        <div className="bg-white rounded-lg shadow-sm border border-slate-200">
          ...
          <div className="px-4 py-4 border-b border-slate-100">
            ...
            <div className="border-t border-slate-100 my-4 pt-4">
              <h4 className="text-xs font-bold text-slate-900 mb-3">Advertising Pricing Settings</h4>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-semibold text-slate-700 mb-1.5">
                    Daily Banner Price (INR) <span className="text-red-500">*</span>
                  </label>
                  <input
                    type="number"
                    min="0"
                    placeholder="Enter Daily Price"
                    value={formData.adBannerDailyPrice}
                    onChange={(e) => handleInputChange("adBannerDailyPrice", e.target.value)}
                    className="w-full px-3 py-2 text-xs border border-slate-300 rounded-lg bg-white focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                  />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-slate-700 mb-1.5">
                    Monthly Banner Price (INR) <span className="text-red-500">*</span>
                  </label>
                  <input
                    type="number"
                    min="0"
                    placeholder="Enter Monthly Price"
                    value={formData.adBannerMonthlyPrice}
                    onChange={(e) => handleInputChange("adBannerMonthlyPrice", e.target.value)}
                    className="w-full px-3 py-2 text-xs border border-slate-300 rounded-lg bg-white focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                  />
                </div>
              </div>
            ...
            <div className="border-t border-slate-100 my-4 pt-4">
              <h4 className="text-xs font-bold text-slate-900 mb-3">Global Radius Settings</h4>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-semibold text-slate-700 mb-1.5">
                    User → Restaurant Radius (KM) <span className="text-red-500">*</span>
                  </label>
                  <input
                    type="number"
                    min="0.1"
                    step="0.1"
                    placeholder="e.g. 10"
                    value={formData.userVisibilityRadius}
                    onChange={(e) => handleInputChange("userVisibilityRadius", e.target.value)}
                    className="w-full px-3 py-2 text-xs border border-slate-300 rounded-lg bg-white focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                  />
                  <p className="text-[10px] text-slate-500 mt-1">How far a user can be from a restaurant and still see and order from it.</p>
                </div>
                <div>
                  <label className="block text-xs font-semibold text-slate-700 mb-1.5">
                    Restaurant → Delivery Boy Radius (KM) <span className="text-red-500">*</span>
                  </label>
                  <input
                    type="number"
                    min="0.1"
                    step="0.1"
                    placeholder="e.g. 10"
                    value={formData.deliveryBoyRadius}
                    onChange={(e) => handleInputChange("deliveryBoyRadius", e.target.value)}
                    className="w-full px-3 py-2 text-xs border border-slate-300 rounded-lg bg-white focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                  />
                  <p className="text-[10px] text-slate-500 mt-1">From the restaurant's location, how far away a delivery boy can be and still get called for that order.</p>
                </div>
              </div>
            </div>
