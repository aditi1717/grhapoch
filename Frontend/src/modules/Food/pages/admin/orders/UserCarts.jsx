...
function CartBillBreakdown({ pricing, fallbackSubtotal = 0 }) {
  const subtotal = Number(pricing?.subtotal ?? fallbackSubtotal) || 0
  const deliveryFee = Number(pricing?.deliveryFee) || 0
  const platformFee = Number(pricing?.platformFee) || 0
  const gstCharges = Number(pricing?.tax) || 0
  const discount = Number(pricing?.discount) || 0
  const total = Number(pricing?.total) || Math.max(0, subtotal + deliveryFee + platformFee + gstCharges - discount)
  ...
  const hasCoupon = Boolean(pricing?.couponCode || pricing?.appliedCoupon?.code)
  const itemDiscountAmount = hasCoupon && discount > 0 ? discount : 0
    ...
    <div className="rounded-xl border border-slate-200 bg-white p-4">
      ...
      <div className="space-y-2.5">
        ...
        <div className="flex justify-between text-sm">
          ...
          <span className={deliveryFee === 0 ? "font-medium text-emerald-600" : "font-medium text-slate-800"}>
        ...
        <div className="flex justify-between text-sm">
          ...
          <span className="font-medium text-slate-800">{formatMoney(platformFee)}</span>
        </div>
        <div className="flex justify-between text-sm">
...
export default function UserCarts() {
  ...
  const getDisplayTotal = useCallback((cart) => {
    const pricing = cart.pricing || pricingByCartId[cart.id]
                        ...
                        <div className="grid grid-cols-1 xl:grid-cols-5 gap-4">
                          ...
                          <div className="xl:col-span-2">
                            {pricingLoadingId === cart.id ? (
                              ...
                              <CartBillBreakdown pricing={pricing} fallbackSubtotal={cart.subtotal} />
                            )}
                          </div>
                        </div>
                      </div>
