      ...
      <Card className="border border-gray-200 shadow-sm">
        <div className="p-4">
          ...
          {!loading && filteredRequests.length > 0 && (
            <div className="flex flex-col md:flex-row items-center justify-between gap-4 mt-4 pt-4 border-t border-gray-200 px-4 pb-4">
              <div className="flex items-center gap-2">
              ...
              <p className="text-sm text-slate-600">Showing {Math.min(filteredRequests.length, (currentPage - 1) * itemsPerPage + 1)} to {Math.min(currentPage * itemsPerPage, filteredRequests.length)} of {filteredRequests.length} entries</p>
              <div className="flex items-center gap-2">
          ...
          {selectedRequest && (
            <div className="p-6 space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                {/* Variants / Variations */}
                {(() => {
                  const variants = selectedRequest.variants || selectedRequest.variations || [];
                  return variants.length > 0 ? (
                    <div className="col-span-full">
                      <label className="block text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-2">
                        Variants / Variations ({variants.length})
                      </label>
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        {variants.map((v, idx) => (
                          <div key={v.id || v._id || idx} className="flex items-center justify-between p-3 bg-slate-50 rounded-xl border border-gray-100">
