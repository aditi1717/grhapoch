...
export const useDeliveryNotifications = () => {
  ...
  }, [deliveryPartnerId, handleIncomingOrderAlert, isPartnerBusy]);
  ...
  useEffect(() => {
    ...
    socketRef.current.on('new_order', (orderData) => {
      ...
      if (newOrderRef.current) {
        debugLog('Ignoring new_order because another offer is already being shown', {
          currentOrderId: getOrderAlertKey(newOrderRef.current),
          incomingOrderId: getOrderAlertKey(orderData),
        });
        return;
      }
    ...
    socketRef.current.on('new_order_available', (orderData) => {
      ...
      if (newOrderRef.current) {
        debugLog('Ignoring new_order_available because another offer is already being shown', {
          currentOrderId: getOrderAlertKey(newOrderRef.current),
          incomingOrderId: getOrderAlertKey(orderData),
        });
        return;
