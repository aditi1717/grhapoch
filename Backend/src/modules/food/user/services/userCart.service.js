...
async function enrichStoredCartPricing(cart, storedPricing) {
    ...
    if (storedDelivery > 0) return storedPricing;
    ...
    try {
        ...
        const total = Math.max(0, subtotal + recalcDelivery + platformFee + tax - discount);
    ...
    const normalized = await Promise.all(
        carts.map(async (cart) => {
            ...
            if (pricing && Number(pricing.deliveryFee) === 0) {
                pricing = await enrichStoredCartPricing(cart, pricing);
            }
...
export async function getUserCartPricingForAdmin(cartId) {
    ...
    if (cart.pricing && Number(cart.pricing.total) > 0) {
        return enrichStoredCartPricing(cart, cart.pricing);
    }
