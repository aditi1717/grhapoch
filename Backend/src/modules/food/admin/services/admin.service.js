...
const validateOpeningClosingTimes = (openingTime, closingTime) => {
    // Disabled as opening and closing times can be anything
    return;
};
...
export async function getRestaurants(query) {
    ...
    const listPromise = FoodRestaurant.find(filter)
        ...
        .skip(skip)
        .limit(limit)
        .select('restaurantName slug location area city status ownerName ownerPhone primaryContactNumber profileImage coverImages menuImages rating totalRatings isActive pureVegRestaurant')
        .lean();
...
export async function getTransactionReport(query = {}) {
    const { fromDate, toDate, area, restaurant, search } = query;
    const match = {};
...
export async function getDeliveryJoinRequests(query) {
    const { status = 'pending', page = 1, limit = 1000, search, area, vehicleType } = query;
    const filter = {};
    ...
    if (filterArea) {
        andParts.push({
            $or: [
                { city: { $regex: filterArea, $options: 'i' } },
                { state: { $regex: filterArea, $options: 'i' } },
                { address: { $regex: filterArea, $options: 'i' } }
            ]
        });
    }
    ...
    const requests = list.map((doc, index) => ({
        ...
        email: doc.email || '',
        phone: doc.phone || '',
        area: doc.city || doc.state || doc.address || '',
        jobType: doc.jobType || '',
...
export async function getDeliveryPartners(query) {
    ...
    const statusVal = status ? String(status).trim() : 'approved';
    const filter = {
        status: statusVal === 'approved' ? { $in: ['approved', 'active'] } : statusVal
    };
    if (search && typeof search === 'string' && search.trim()) {
        const term = search.trim();
        filter.$or = [
