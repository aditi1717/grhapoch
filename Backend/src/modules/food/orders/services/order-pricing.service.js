...
export function getDeliveryDistanceKm(restaurant, deliveryAddress) {
  return calculateDistanceKm(restaurant, deliveryAddress);
}
...
export async function calculateOrderPricing(userId, dto, options = {}) {
  ...
  let distanceKm = calculateDistanceKm(restaurant, deliveryAddress);
  ...
  const total = round2(
    Math.max(0, subtotal + packagingFee + deliveryFee + platformFee + tax - discount),
  );
